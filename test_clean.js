...
(async () => {
  // كل حالة برقم مختلف عشان ما يتداخل flowState
  const cases = [
    ['96771000001', 'كم مدة التوصيل', 'يرد الشحن صح'],
    ['96771000002', 'كيف أسترجع منتج', 'يرد الاسترجاع صح'],
    ['96771000003', 'ايميل هالات', 'يرد الإيميل صح'],
    ['96771000004', 'كم سعر التيشرت', 'يرد التيشرت صح'],
    ['96771000005', 'هل يشتغلون الجمعة', 'يرد الجمعة صح'],
  ...
  // اختبار تدفق التلف برقم مستقل
  console.log('--- تدفق التلف ---');
  await webhookSim('الشحنة وصلتني تالفة', '96771000099');
  await wait(1200); console.log('1:', await getOut());
  await webhookSim('#1234', '96771000099');
  await wait(1200); console.log('2:', await getOut());
  await webhookSim('صورة المنتج', '96771000099', true);
  await wait(1200); console.log('3:', await getOut());
})();
