...
function last() {
  return new Promise(r => {
    const q = https.request({ host: 'wasilah-wa.onrender.com', path: '/admin/api/messages', method: 'GET', auth: 'admin:RxWa@2026!Admin' }, res => {
      let d = ''; res.on('data', c => d += c); res.on('end', () => { try { const m = JSON.parse(d).filter(x => x.direction === 'out' && x.text); r(m.length ? m[m.length - 1].text : 'NO OUT'); } catch (e) { r('ERR'); } });
    });
    q.end();
  });
}
...
(async () => {
  ...
  out.push('1 (طلب الرقم): ' + (await last()).slice(0, 60));
  await w('#1234', num); await wait(2500);
  out.push('2 (طلب الصورة): ' + (await last()).slice(0, 60));
  await w('هذي صورة', num, true); await wait(2500);
  out.push('3 (تأكيد): ' + (await last()).slice(0, 60));
  ...
  out.push('4 (بعد انتظار + رسالة): ' + (await last()).slice(0, 60));
